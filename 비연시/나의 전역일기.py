...
while True:
    ...
    def story_1():
        ...
        print("{}: 늘 그렇듯 행보관의 짬처리를 하고있을테니 행정반에 있겠지.".format(player))
        ...
        print("{}: 이게 뭐하는거야?".format(player))
        pressEnterToContinue()

        print("{}: 이건 0창에서 받아야 하는것들 품목이랑 수량 정리하는거고 이건 간부들 분기별 중식 예산이야.".format(heroine[0]))
        pressEnterToContinue()

        print("{}: 이야 보급관한테 제대로 짬처리 당했구나.. 힘들겠다".format(player))
        pressEnterToContinue()

        print("{}: 항상 그렇지 뭐.. {} 너는 무슨 일로 왔어?".format(heroine[0], player))
        pressEnterToContinue()
        ...
        print("{}: 그러고보니까 {} 너 전역이 얼마 안남았구나..".format(heroine[0],player))
        pressEnterToContinue()

        print("{}: 그러게 벌써 시간이 이렇게 됐네".format(player))
        pressEnterToContinue() 
        ...
        print("{}: 아 벌써 청소시간이네 가야겠다".format(player))
        pressEnterToContinue()
        ...
        if relationshipPoint == 0:
            ...
            print("{}: 나 간다 남은 군생활 잘 해라.".format(player))
            pressEnterToContinue()

            print("{}: 잘가 {} 가끔 생각나면 연락할게".format(heroine[0],player))
            pressEnterToContinue()

            print("{}: 그래 언젠가 인연이 닿으면 보자.".format(player))
            pressEnterToContinue()
            ...
            print("{}: 그래 ㅋㅋ 고생해라".format(player))
            pressEnterToContinue()      
            ...
            print("{}: 야~ 이제 들어가 뭐하러 여기까지 나왔어".format(player))
            pressEnterToContinue()
            ...
            print("{}: 아 새끼 안울려고 했는데 눈물나게..".format(player))
            pressEnterToContinue()
            ...
            print("{}: 어.. 저기.. {}?".format(player, heroine[0]))
            pressEnterToContinue()
    ...
    def story_2():
        print("{}: 그래 오늘은 {}이나 만나러 가자.".format(player, heroine[1]))
        pressEnterToContinue()
        ...
        if conversationSelect == 1:
            ...
            print("{}: 무슨 드라마 보는데?".format(player))
            pressEnterToContinue()
            ...
            print("{}: ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ 이 집 리액션 맛있네".format(player))
            pressEnterToContinue()

            print("{}: 아 눈뽕 뭔데 ㅡㅡ".format(heroine[1]))
            pressEnterToContinue()

            print("{}: 군인이라면 섬광탄에 대한 면역을 기르기 위해 이런 훈련이 필요하다고 생각해.".format(player))
            pressEnterToContinue()
            ...
            print("{}: 사람이 좀 밝은곳에서 생활하고 그래야지".format(player))
            pressEnterToContinue()

            print("{}: ?? 너도 맨날 불꺼놓고 어둡게 해놓잖아".format(heroine[1]))
            pressEnterToContinue()

            print("{}: 나는 나고 너는 너지 어떻게 같겠니?".format(player))
            pressEnterToContinue()

            print("{}: 내로남불 지리네 가끔 보면 넌 진짜 미친놈인 것 같아.".format(heroine[1]))
            pressEnterToContinue()

            print("{}: 그게 내 매력이 아닐까-☆".format(player))
            pressEnterToContinue()
        ...
        print("{}: 그냥~ 이제 게임방도 질려서 사람들이랑 얘기 나누는 낙에 사는거지 뭐..".format(player))
        pressEnterToContinue()
